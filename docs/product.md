...
四項基準：集中維護但不全部載入；允許探索但不無限擴張；保留摘要但不丟掉證據；意外中斷可自動恢復，無法接續時可自動重跑；重跑不代表盲目重複外部操作，詳細分界見 [workflow](design/workflow.md#取消中斷與認證失效)。新增流程或角色要說明解決的具體問題與成本。
...
| 互動 | Issue／留言區式：下目標、補資訊、回答必要問題、採納成果、要求修改或終止。回答接續與狀態可見性見 [workflow](design/workflow.md#必要問題的回答接續)；不要求即時顯示完整執行過程。 |
| 配置 | 平台集中維護共用內容，專案提供差異、必要限制與探索入口；能力可發現不等於強制使用。跨專案[探索要求與閱讀自主性](design/configuration.md#索引與閱讀自主性)，以及[依工作指引驗證與回報](design/workflow.md#依工作指引驗證與回報)，分別由相關文件維護。 |
...
05 後續明確補入的 [code 獨立 review 與 PR 修改交付](design/workflow.md#code-工作的獨立-review)，作為上述完成界線之外的行為要求；角色隔離與修正流程仍待設計。

## 工作接續與恢復

05 的已確認互動基準與取捨，主要維護於 workflow：[必要問題的回答接續](design/workflow.md#必要問題的回答接續)、[PR 後修改](design/workflow.md#pr-後修改與交付)、[執行中改需求、立即終止與意外恢復](design/workflow.md#取消中斷與認證失效)。主動拋棄 run 與意外中斷分開，允許後者自動接續或必要時重跑，不意味任意程序救援。

[降低新一輪探索成本](design/workflow.md#工作內的探索接手)是已確認目的；工作內共用暫存區與[按角色初始化配置](design/configuration.md#當次-agent-角色與配置初始化候選)仍是候選。它們不增加永久角色群、完整對話共享或每輪自由重寫規則的既定要求。
...
- 所有 code 相關工作須由另一個 agent review，依 [workflow](design/workflow.md#code-工作的獨立-review)；一般測試可由程式執行。非 code 工作是否 review、具體流程與修正輪數仍未定，不因角色要求已明確就採納完整候選骨架。
